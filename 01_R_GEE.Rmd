---
title: "Geomatics and Earth Surface Modelling part 2"
...
Factors are a smart way that R uses to create classes and save space:

![](images/clipboard-322141056.png)
...
file.size.difference <-
  file.size("countries.rda") / file.size("countries.factors.rda")
message(
  "File with factors is " ,
  round(file.size.difference, 2),
  "X smaller than file with full characters"
)
...
Installed and loaded when needed.

```{r eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
install.packages("terra")
library(terra)

# or can be used without loading
# terra::rast(...)
```
...
WE WILL ANALYSE Temperature and NDVI indices

# Spatial Data QGIS / GEE

## Draw a study area in QGIS

We will learn to draw our study area, upload assets in GEE, share assets, clip data in GEE and download for further processing in R/QGIS

Video on adding new polygon with QGIS <https://mediaspace.unipd.it/media/QGIS+Tutorial+02+-+Digitizing+new+vectors+-+Snapping/1_wdtai8ua>

## ASSIGNMENT 1

Share the asset and add the link to a shared document.

# Programming in GEE

## Introduction

-   Introduction <https://developers.google.com/earth-engine/tutorials/tutorial_js_01>
-   Objects <https://developers.google.com/earth-engine/tutorials/tutorial_js_02>

See the 3 videos in the playlist <https://mediaspace.unipd.it/playlist/dedicated/1_hxjx1zie/> for material on contents in the next sections.

### Upload and share assets in GEE

Upload your polygon as asset to GEE <https://developers.google.com/earth-engine/guides/asset_manager>

### Adding the polygon asset to your script

![](images/clipboard-661012625.png)

### Adding a Landsat 9 image to your script

![](images/clipboard-2363019282.png)

### Filtering the Landsat 9 image

![](images/clipboard-969334674.png){width="422"}

## Functions

-   Functions <https://developers.google.com/earth-engine/tutorials/tutorial_js_03>
...
-   View asset
-   Add a filtered Landsat imagecollection
-   Map and Reduce concept to remove clouds and calculate NDVI
-   clip and export final NDVI in the shared folder as TIFF file
...
Here we load two rasters, and we check some info about them. We see that the raster object has **attributes** and **methods** , basically attributes are variables and methods are functions that are availabe for that object.

For example the function "res" is a method of the "SpatRaster" object as we can see here.

```{r eval=FALSE }
library(terra)
dem <- terra::rast("pathToDEM") 
temp <- terra::rast("pathToTemperature") 
...
Use images exported from google earth engine and processed in R to:
